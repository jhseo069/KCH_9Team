...
### 5-9. 통합 구성 (2026-09-18)

> 원래 작업 계획서에는 이 절 번호가 "5-8"로 적혀 있었으나, 그 사이 §5-8(Supabase
> 자동정지 방지)이 먼저 병합되어 번호가 겹친다. 번호만 5-9로 밀어서 기록한다.

강경미·황정연 두 팀원의 도구를 결과 탭으로 통합한 작업의 기록이다. 통합 설계 근거는
`나만의 AI Agent/사업부지_통합에이전트/2026.09.18_통합에이전트_클라우드배포_설계_v1.md`.

**필지 탭: 실시간 값과 사전 적재 값이 섞여 있다.** `public/parcel_view.js`가 행을
만들 때 두 출처를 합친다 — 용도지역은 지금까지처럼 Supabase `zoning`을 실시간
조회한 값이 권위 있는 출처이고, **지목·면적·개별공시지가는 사전 적재한 `parcels`
테이블이 유일한 출처**다(설계서 §5.2, §5.3). eum.go.kr 라이브 조회가 서버에서
막혀 있어(§4) 이 세 값은 애초에 실시간으로 가져올 방법이 없다 — "느려서 캐싱한다"가
아니라 "실시간 경로 자체가 없다"는 뜻이다. `parcels`에 해당 좌표 반경 50m 안의 행이
없으면 실패로 취급하지 않고 조용히 "미제공"을 낸다(`public/index.html`의
`fetchParcel`) — 적재 안 된 것과 조회 실패는 다른 상태이기 때문이다.

**`scripts/load_parcels_to_supabase.py`는 로컬에서만, 사람이 손으로 실행한다.**
등기부등본처럼 클라우드에서 재생성할 수 없는 출처가 섞인 토지조서 산출물
(`ingest/parcel_contract.py`가 Supabase 적재 행으로 변환)을 올리는 배치이기
때문에, 웹앱이 대신 실시간으로 만들어낼 수 있는 데이터가 아니다. 실행 시점:
- **시연 직전 1회** — 시연에 쓸 부지의 토지조서가 최신인지 확인 후 적재
- **새 부지가 추가될 때마다** — 강경미 팀원이 토지조서를 새로 만든 부지는 이 스크립트를
  돌리기 전까지 필지 탭에서 "미제공"으로 보인다. 적재를 잊으면 화면이 깨지는 게
  아니라 조용히 비어 보이므로, 새 부지를 다룰 때마다 이 스크립트를 돌렸는지 스스로
  체크하는 습관이 필요하다

**`ingest/parcel_contract.py`의 `lon`/`lat` 생산자 간극은 해소됨 (2026.09.18).** 강경미
`토지조서자동화` 프로젝트(이 저장소 밖, 별도 프로젝트, `kangkyoungmi/my-webpage` 예정)의
`ParcelRecord`에 `경도`/`위도` 필드를 추가하고, `main.py::process_parcel`이
`vworld_client.search_pnu`가 원래 받아오던 x/y(예전에는 캐시 파일에만 남고 버려졌음)를
그 필드에 붙이도록 고쳤다. 새 CLI 플래그 `--json-output`을 쓰면 `src/export_json.py`가
이 저장소의 `scripts/load_parcels_to_supabase.py`가 그대로 읽을 수 있는 JSON(키 이름
`lon`/`lat`, PNU 없는 레코드는 기본키가 없어 제외하고 건수를 경고)을 출력한다.
마명리 12필지 실측 데이터로 종단 검증 완료 — 좌표 누락 0건.
`scripts/load_parcels_to_supabase.py`의 좌표-누락 경고(`load_records`)는 이제 정상
경로에서는 트리거되지 않아야 하며, 트리거된다면 그 자체가 회귀 신호다.
...
`20260916_신재생공사비산출기_v1.3.html` → `..._v1.4.html`). 부분 수정으로 맞추고
싶은 유혹이 있어도 참아야 하는 이유는, 황정연 님이 자신의 계산기를 독립적으로 계속
버전업하고 있어서 여기서 손대면 다음 원본 갱신 때 그 수정분을 다시 반영해야 하는
부담이 생기기 때문이다. 파일명이 바뀌면 `public/index.html`의 `#econFrame` `src`
속성과 `public/econ/README.md`도 같이 갱신해야 한다(둘 다 옛 파일명을 그대로 물고
있으면 iframe이 404가 난다). 값 전달은 postMessage가 아니라 같은 출처 DOM 접근
(`public/econ_bridge.js`의 `buildEconContext`)이며, 사업명 입력란 하나만 채운다 —
긍장·전압 등 계통연계 값은 부지 조회 데이터로 근거 있게 못 채우므로 사람 입력으로 둔다.

**시연 전에는 반드시 `scripts/demo_smoke_check.py <배포URL>`을 돌린다.** 판정
API, 필지 조회(`parcel_at`), 시연용 부지 목록(`demo_sites`) 세 가지를 순서대로
확인하고, 하나라도 실패하면 종료 코드 1을 낸다(자세한 항목은 스크립트 참고).

> **주의 — 이 안전망은 그 자체로 아직 완성이 아니다.** `scripts/demo_sites_seed.sql`의
> 시연용 부지 3건은 **좌표가 자리표시자(placeholder)다.** 이 파일을 만든 세션에는
> Supabase 접속 정보가 없어 `parcels` 테이블의 실제 좌표를 확인할 수 없었기 때문이다.
> 자리표시자 그대로 실행하면 `parcel_at()`이 그 좌표 반경 50m 안에서 아무 필지도
> 못 찾아 필지 탭이 "미제공"으로 뜬다 — 이 통합 작업 전체가 막으려던 바로 그 시연
> 사고가 안전망 데이터 자체에서 재현되는 셈이다. **사람이 다음을 먼저 해야 한다:**
> 1. `scripts/load_parcels_to_supabase.py`로 시연용 부지 3건의 토지조서를 먼저 적재
> 2. Supabase SQL Editor에서
>    `select pnu, 소재지, 지번, st_x(geom) as lon, st_y(geom) as lat from parcels where 소재지 like '%<주소 일부>%';`
>    로 실제 좌표를 확인
> 3. `scripts/demo_sites_seed.sql`의 세 행의 `lon`/`lat`을 그 값으로 바꾼 뒤 실행
>
> 이 교체 없이 `demo_smoke_check.py`를 돌리면 "필지 조회" 항목이 실패로 나와야
> 정상이다 — 통과했다면 오히려 좌표를 아직 안 바꿨는지 다시 확인할 것.
