...
def test_괄호가_안_맞으면_쪼개지_않고_통째로_반환한다():
    # 여는 괄호만 있고 닫는 괄호가 없으면 원문이 잘렸거나 손상된 것이다. 이런
    # 문자열을 그대로 정규식에 넣으면 괄호 안 쉼표까지 구분자로 쪼개져 아무도
    # 쓰지 않은 규제 항목이 생긴다
    assert split_regulations("A(open, no_close, 규제2") == [
        "A(open, no_close, 규제2"
    ]


def test_괄호가_여러_개여도_맞으면_정상적으로_쪼갠다():
    assert split_regulations("가축사육제한구역(일부), 문화재보호구역(전부)") == [
        "가축사육제한구역(일부)", "문화재보호구역(전부)"
    ]
